...
import agh.cs.po.Visualization.MapVisualizer;
...
public class SteppeJungleMap implements IWorldMap,IPositionChangeObserver {
    ...
    public Map<Vector2d,LinkedList<Animal>> animals = new HashMap<>();
        ...
        //visualization
    private MapVisualizer v;


    public SteppeJungleMap(int width, int height, int jungleWidth, int jungleHeight, int grassProfit, int dayCost,int energyLimitToCopulation) {
    ...
    private Vector2d toNoBoundedPosition(Vector2d position) {
        int newX;
        int newY;
        ...
        if (position.y < lowerLeft.y) {
            newY = (width - Math.abs(position.y % height)) % width;
        } else {
    ...
    private boolean removeAnimal(Animal a,Vector2d position2){
        ...
        if(l == null) throw new IllegalArgumentException("Animal" + a.getPosition() + " already not exist in the map");
        else if(l.size() == 0 )throw new IllegalArgumentException("Animal" + a.getPosition() + " already not exist in the map empty list");
        else {
            l.remove(a);
        }
        return true;
    ...
    void spawnGrass(){
        ...
        while((double) toMuchTimes < (double) 2*((double)jungleSize/(double)steppeSize)*mapSize){
            ...
            if(grass.get(newGrass) == null && !(newGrass.precedes(lowerLeftJungle) && newGrass.follows(upperRightJungle))){
                place(new Grass(newGrass));
                break;
            }
        toMuchTimes++;
